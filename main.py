...
from interaction_agent.tester.tester import Tester
...
#     model=_model, api_key=SecretStr(os.getenv("GEMINI_API_KEY"))
# )
...
async def main(url=None):
    ...
    prompt = "Buy 2 backpacks."
    full_prompt = f"{context}\n{prompt}"
    tester = Tester(
        model=_model,
        context_prompt_path="Prompts/Analysis/context.txt",
        max_steps=10,
    )
    if url:
        result = await tester.test(["Buy 2 backpacks.", "Buy 2 t-shirts."], url)
        await tester.close()  # Explicitly close the tester resources
